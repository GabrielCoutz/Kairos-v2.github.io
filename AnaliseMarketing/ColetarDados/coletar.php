...
<head>
    ...
    <?php
      ...
      if(!isset($_SESSION['email_padrao']) && !strpos($protocol . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'],hash("sha512", 'erro=true'))){
        header("Refresh:0; url=ColetadeDados".'?'.hash("sha512", 'erro=true'));
        exit;
      }
    ?>
</head>
