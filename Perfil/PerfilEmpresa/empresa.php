...
<head>
    ...
    switch (true) {
        ...
        case empty($select_empresa['ramo']) && !strpos($protocol . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], hash("sha512", 'dados_empresa=false')) && !strpos($protocol . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], hash("sha512", 'erro=true')):
            header("Refresh:0; url=empresa" . '?' . hash("sha512", 'dados_empresa=false'));
            exit;
            break;
    }
        ...
        <div class="barra-lateral">
            <nav aria-label="Navegação Lateral">
                <ul class="nav-lateral">
                    <li class='nav-lateral-ativo'><svg width="24" height="24" viewBox="0 0 24 24" fill="#000000" xmlns="http://www.w3.org/2000/svg">
                        ...
                        </svg><a href="empresa" tabindex="0">Perfil da Empresa</a></li>
                    ...
                    <li><i aria-hidden="true" class="gg-user"></i>
                        <a href="../usuario" tabindex="0">Perfil</a>
                    </li>
            ...
            <form method="POST" action="assets/php/atualizar_empresa" class="container perfil empresa">
                <div class="fundo-dados">
                    <div class="form-caixa">
                        ...
                        <input type="text" name="nome_empresa" id="nome_empresa" maxlength="50" value='<?= $select_empresa['nome'] ?>' aria-controls="nome_empresaAlert" onkeyup="apenasLetras(this)">
                        <div id="nome_empresaAlert" role="alert"></div>
                    </div>
                    <div class="form-caixa">
                        <label for="nome_fantasia">Nome Fantasia</label>
                        <input type="text" name="nome_fantasia" id="nome_fantasia" maxlength="50" value='<?= $select_empresa['nome_fantasia'] ?>' aria-controls="nome_fantasiaAlert">
                ...
                <div class="fundo-dados info-1">
                    <div class="dados-coluna">
                        <div class="form-caixa">
                            ...
                            <input type="tel" name="cep_empresa" id="cep_empresa" value='<?= $select_empresa_endereco['cep'] ?>' aria-controls="cep_empresaAlert" onkeypress="$(this).mask('00.000-000')" onkeyup="lerCEP(this)">
                            <div id="cep_empresaAlert" role="alert"></div>
                        </div>
                        <div class="form-caixa">
                            <label for="numero_empresa">Número</label>
                            <input type="tel" name="numero_empresa" id="numero_empresa" value='<?= $select_empresa_endereco['numero'] ?>' aria-controls="numero_empresaAlert">
                ...
                <div class="botoes">
                    <button class="btn primario" type="button" id="salvarbtn" onclick="salvar()" disabled="true">Salvar</button>
                    <button class="btn secundario" type="reset" id="cancelarbtn" onclick="cancelar()" disabled="true">Cancelar</button>
                </div>
            </form>
        </div>
    </div>
